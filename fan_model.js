...
function drawFanMotor() {
    ...
    if (oscillationEnabled) {
        mat4.rotateY(modelViewMatrix, modelViewMatrix, oscillationAngle * Math.PI / 180);
    }
...
function drawFanBlades() {
    ...
    if (bladeRotationEnabled) {
        mat4.rotateZ(modelViewMatrix, modelViewMatrix, bladeAngle * Math.PI / 180);
    }
...
function animate(currentTime) {
    ...
    if (oscillationEnabled) {
        ...
        oscillationAngle = Math.sin(currentTime * oscillationFrequency) * maxOscillationAngle;
    }
    ...
    // Update UI stats
    document.getElementById('currentOscillation').textContent = 
        oscillationAngle.toFixed(1) + '°';
    document.getElementById('currentBlade').textContent = 
        bladeAngle.toFixed(0) + '°';
